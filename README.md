...
젠킨스는 localhost로 실행되기 때문에 깃허브 레포 변화에 따른 build를 재생성하려면 기본적으로 외부에서 로컬에 알림을 보낼 수 있어야한다.

그러기 위해 대제목 1번에서는 ngtok을 활용 local에서 돌아가는 젠킨스에 깃허브가 접근할 수 있게 해준다.

대제목 2에선 ngrok의 역할을 aws가 해준다.
